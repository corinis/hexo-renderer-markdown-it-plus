...
module.exports = function(data, options) {
    ...
    md = plugins.reduce(function (md, pugs) {
        if(pugs.enable) {
            ...
            } else if(pugs.name == 'markdown-it-container') {
                if(pugs.options == null) pugs.options = {}
                if(!pugs.options.name) {
                    pugs.options.name = 'container';
                }
                return md.use(require('markdown-it-container'), pugs.options.name, pugs.options);
	    } else {
